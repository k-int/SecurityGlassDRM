...
class ImageIdentifyInterface
{
	@javax.annotation.PostConstruct
	def init() {
		log.debug("ImageIdentifyInterface init method called");
	}
	 ...
	 */
	def getImageInfo(image_file_name, imagemagick_abs_path) {
		log.debug("ImageIdentifyInterface::getImageInfo called with image_file_name: ${image_file_name} and imagemagick_abs_path: ${imagemagick_abs_path}");
		...
		{
			log.error("Error returned whilst looking up details for the file ${image_file_name} : ${identify_file_proc.err.text}")
			throw new ImageProcessingException(identify_file_proc.err.text)
		}
