...
class ImageWatermarkService {
	...
	def init() {
		log.debug("ImageWatermarkService init called");
	}

	/**
	 * Embed the identified properties as XMP attributes in the given file.
	 */
	def watermark(image_file_name, watermark_text) {
		log.debug("Watermarking file ${image_file_name} with text ${watermark_text}");
		...
		log.debug("Watermark conversion commands: ${conv_cmd_arr} | ${wm_cmd_arr}");
