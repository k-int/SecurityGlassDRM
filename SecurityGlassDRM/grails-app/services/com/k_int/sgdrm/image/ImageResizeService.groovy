...
class ImageResizeService {
	...
	def init() {
		log.debug("In the ImageResizeService init method");
	}

	/**
	 * Embed the identified properties as XMP attributes in the given file.
	 */
	def resize(image_file_name, resize) {
		log.debug("Resizing file ${image_file_name} to ${resize}");
		...
		// TODO - check that the above actually considers the resize argument...
	 ...
	 */
	def resize(input_file_name, output_file_name, resize_height, resize_width, ignore_aspect_ratio, imagemagick_abs_path) {

		log.debug("Resizing file ${input_file_name} to ${resize_height ? resize_height : 'unspecified'} x ${resize_width ? resize_width : 'unspecified'} (h x w)");
		...
		{
			def image = ImageIdentifyInterface.getImageInfo(input_file_name, imagemagick_abs_path)
			...
			{
				throw new ImageProcessingException("Failed to retrieve the height of the original image")
			}
		}
		else if(resize_width) //% scale by width
		{
			...
			{
				throw new ImageProcessingException("Failed to retrieve the width of the original image")
			}
		}
		else //neither height nor width specified so cannot resize
		{
			throw new ImageProcessingException("Unable to resize image as neither a height nor width was supplied")
		}

		if(resize_value)
		{
			log.debug("Actually resizing with resize_value: ${resize_value}");
			...
			{
				throw new ImageProcessingException(resize_cmd.err.text)
			}
		}
	}
}